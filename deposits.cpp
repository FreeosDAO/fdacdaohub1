ACTION fdacdaohub1::opendeposit(name account, name ram_payer, extended_asset amount) {
...
ACTION fdacdaohub1::withdraw(name account, extended_asset amount) {
...
ACTION fdacdaohub1::movedeposit(name from, name to, extended_asset amount) {
  require_auth(from);
  check(amount.quantity.amount > 0, "Amount must be greater then zero");
...
void fdacdaohub1::sub_deposit( const name& account, const extended_asset& value) {
    deposits_table _deposits( get_self(), account.value);
...
void fdacdaohub1::add_deposit( const name& account, const extended_asset& value){
    deposits_table _deposits( get_self(), account.value);
