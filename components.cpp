    ...
    ACTION fdacdaohub1::compreg(name owner, string src, checksum256 hash, string info_json ){
        require_auth(owner);
    ...
    ACTION fdacdaohub1::compupdatesr(uint64_t comp_id, string new_src, checksum256 new_hash ){
        components_table _components(get_self(), get_self().value );
    ...
    ACTION fdacdaohub1::compupdatein(uint64_t comp_id, string info_json){
        components_table _components(get_self(), get_self().value );
        auto itr = _components.find(comp_id);
        check(itr != _components.end(), "Component with this id doesn't exists.");
        require_auth(itr->owner);
    ...
    ACTION fdacdaohub1::compapprove(uint64_t comp_id , checksum256 hash ){
    ...
    ACTION fdacdaohub1::compunapprov(uint64_t comp_id ){
        require_auth(get_self() );
        components_table _components(get_self(), get_self().value );
        auto itr = _components.find(comp_id);
        check(itr != _components.end(), "Component with this id doesn't exists.");
    ...
    ACTION fdacdaohub1::compdelete(uint64_t comp_id ){
        require_auth(get_self() );
        components_table _components(get_self(), get_self().value );
        auto itr = _components.find(comp_id);
        check(itr != _components.end(), "Component with this id doesn't exists.");
